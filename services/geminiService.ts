...
const generateInstrumentalPrompt = (options: GeneratePromptOptions): string => {
    const { genre, mood, includeSolfeggio, influenceGenre, instruments } = options;
...
- **Musical Foundation:** Specify an exact **BPM** and a **Musical Scale** (e.g., C# Minor, A Lydian).
...
const generateFullSongPrompt = (options: GeneratePromptOptions): string => {
    const { genre, mood, includeSolfeggio, vocalStyle, atmosphere, lyrics, structure, influenceGenre, instruments, lyricalConcept } = options;
