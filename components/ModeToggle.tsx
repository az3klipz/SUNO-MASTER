...
const ModeToggle: React.FC<ModeToggleProps> = ({ selectedMode, onSelectMode }) => {
  const modes: { name: PromptMode; description: string; icon?: React.ReactNode }[] = [
    ...
    { name: 'Analyze Track', description: 'Analyze a song to use its style as a starting point.', icon: <MusicNoteIcon className="w-6 h-6 mr-2 text-green-400" />},
    { name: 'Video Treatment', description: 'Generate a music video concept from a finished song prompt.', icon: <VideoCameraIcon className="w-6 h-6 mr-2 text-cyan-400" /> },
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {modes.map((mode) => (
        ...
        >
          ...
          <p className="mt-1 text-gray-300 text-sm">{mode.description}</p>
        </button>
      ))}
    </div>
  );
